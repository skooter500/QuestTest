...
public class PhysicsSpawner : MonoBehaviour {
    ...
    void CreateWall(int width, int height)
    {
        ...
        for (int row = 0; row < height; row++)
        {
            for (int col = -halfw; col < halfw; col++)
            {
                ...
                cube.transform.position = player.transform.TransformPoint(new Vector3(x, y, 5));
    ...
    void CreateTentacle(int segments)
    {
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            cube.transform.rotation = player.transform.rotation;
            cube.transform.position = player.transform.TransformPoint(new Vector3(0, 1, z));
            cube.GetComponent<Renderer>().material.color = Color.HSVToRGB(Random.Range(0.0f, 1.0f), 1, 0.8f);
    ...
    void CreateTower(float radius, int height, int segments)
    {
        ...
        for (int h = 0; h < height; h++)
        {
            for (int i = 0; i < segments; i++)
            {
                ...
                GameObject cube = GameObject.Instantiate<GameObject>(cubePrefab);
                cube.transform.rotation = player.transform.rotation * Quaternion.AngleAxis(theta * Mathf.Rad2Deg, Vector3.up);
                cube.transform.position = player.transform.TransformPoint(new Vector3(x, h, z));
                cube.GetComponent<Renderer>().material.color = Color.HSVToRGB(Random.Range(0.0f, 1.0f), 1, 0.8f);
            }
        }
    }
